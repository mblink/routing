...
import cats.Monad // 0.23, 1.0.0-M31
...
object syntax {
  ...
  implicit class Http4sReverseUriOps(val uri: ReverseUri) extends AnyVal {
    def toHttp4s: h.Uri =
      h.Uri(path = h.Uri.Path.unsafeFromString(uri.path), query = new Http4sReverseQueryOps(uri.query).toHttp4s) // 0.22, 0.23, 1.0.0-M31
  ...
  implicit class Http4sRouteObjectOps(private val route: Route.type) extends AnyVal {
    ...
    def httpRoutes[F[_]: Monad]( // 0.23, 1.0.0-M31
